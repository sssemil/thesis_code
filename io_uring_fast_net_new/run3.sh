...
run_test() {
    ...
    local pin_threads=$4
    local connections_per_thread=4
    local thread_count=$5
    local server_addr="10.0.1.144"
    local subdir=$6
    ...
    echo "Completed run with ALLOC_PIN=${alloc_pin}, ENABLE_NAGLE=${enable_nagle}, INCREASE_SOCKET_BUFFERS=${increase_socket_buffers}, PIN_THREADS=${pin_threads}, THREAD_COUNT=${thread_count}"
    echo ""
}
...
perform_tests() {
    ...
    for ((i=0; i<$combinations; i++)); do
        ...
        subdir="A${alloc_pin}N${enable_nagle}S${increase_socket_buffers}P${pin_threads}"
        mkdir "${subdir}"

        cd "${subdir}"
        for thread_count in "${THREAD_COUNTS[@]}"; do
            run_test "$alloc_pin" "$enable_nagle" "$increase_socket_buffers" "$pin_threads" "$thread_count" "$subdir"
        done
        cd ..
    done
